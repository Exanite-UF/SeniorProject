...
vec3 waveletIteration(sampler2D inputColor, sampler2D inputVariance, sampler2D inputNormal, sampler2D inputClipPosition)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            vec2 offset = vec2(i - 2, j - 2) * vec2(pow(2, iteration));
...
void main()
{
    waveletIteration(inputColor, inputVariance, inputNormal, inputClipPosition); // This needs to set the historical color data as well
}
